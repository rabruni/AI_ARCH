...
#
# Usage:
#   ./run.sh                     # Front-door agent demo (default)
#   ./run.sh -f, --frontdoor     # Front-door agent demo
#   ./run.sh -i, --interactive   # Full interactive mode (Locked System CLI)
...
Modes:
  (default)              Front-door agent demo
  -f, --frontdoor        Front-door agent demo (routing/signals)
  -i, --interactive      Full interactive chat mode
  -d, --debug            Debug mode (shows full system state)
  -m, --message MSG      Process single message and exit
  -s, --setup            Run setup validation
  -t, --test             Run tests (pytest)
...
Examples:
  $(basename "$0")                           # Front-door agent demo
  $(basename "$0") -i                        # Full interactive session
...
case $MODE in
    frontdoor)
        if [[ $VERBOSE == true ]]; then
            log_info "Starting front-door agent demo..."
        fi
        exec python3 "$SCRIPT_DIR/demo_front_door.py"
        ;;
    ...
    test)
        ...
        exec pytest "$SCRIPT_DIR/tests/" -v
        ;;
